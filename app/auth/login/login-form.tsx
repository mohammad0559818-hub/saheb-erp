...
export function LoginForm() {
  ...
  async function signIn() {
    ...
    try {
      ...
      if (error) setMessage(error.message);
        ...
        if (token) document.cookie = `saheb_access_token=${token}; path=/; max-age=28800; SameSite=Lax`;
  ...
  return (
    <form className="w-full max-w-md rounded-[2rem] bg-white p-8 shadow-xl" onSubmit={(event) => { event.preventDefault(); void signIn(); }}>
      ...
      <label className="mt-6 block text-sm font-bold">البريد الإلكتروني<input className="mt-2 w-full rounded-xl border p-3" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="admin@saheb.local" required /></label>
